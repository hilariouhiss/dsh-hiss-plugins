...
test/               node:test 单元测试（真实解析 skills/，并守住 §3.4 依赖契约）
```
...
> `@deepseek-ai/dsh-skill` / `@deepseek-ai/dsh-llm` 由**正在运行的 dsh 安装**提供，本包只在 `peerDependencies` 里声明（`devDependencies` 写精确版本供本仓安装与测试）。**不要**把它们写进 `dependencies` —— 那会让 pnpm 往 profile 里塞一份自己的副本，遮蔽安装目录里的同名包，两代混装会让整棵插件树启动失败。升级 dsh 后先跑 `node ../scripts/smoke-profile.mjs` 再发版。
>
