...
export const PROVIDER_NAME = "openspec";
const SOURCE = "openspec-plugin";
/**
 ...
 * user who drops their own `openspec-*` skill into `~/.dsh/skills` still wins.
 */
const RANK = 350;
...
/**
 * Global-layer skill provider that exposes the bundled OpenSpec workflow skills.
 *
 ...
 * demand.
 */
export class OpenspecSkillProvider {
	name = PROVIDER_NAME;
	#ctx;
	...
	async list(options) {
		...
		try {
			...
			this.#ctx.logger.warn(`openspec: failed to list skills: ${String(error)}`);
	...
	async #readSkill(path) {
		...
		try {
			...
			this.#ctx.logger.warn(`openspec: failed to read ${path}: ${String(error)}`);
			return undefined;
		}
